...
public class AdvancedManipulations {
    ...
    public static PixelPicture reducePalette(PixelPicture pic, int numColors) {
        ...
        for (int col = 0; col < w; col++) {
            for (int row = 0; row < h; row++) {
                if (m.contains(src[row][col])) {
                    m.put(src[row][col], m.getValue(src[row][col]) + 1);
                } else {
                    m.put(src[row][col], 1);
                }
            }
        ...
        Pixel[] originalColor = new Pixel[m.size()];
        originalColor = m.getSortedPixels();
        Pixel[] limitedColor = new Pixel[numColors];

        for (int i = 0; i < numColors; i++) {
            limitedColor[i] = originalColor[i];
        }
        ...
        for (int col = 0; col < w; col++) {
            for (int row = 0; row < h; row++) {
                ...
                for (int i = 0; i < numColors; i++) {
                    if (src[row][col].distance(limitedColor[i]) < minDistance) {
                        minDistance = src[row][col].distance(limitedColor[i]);
                        index = i;
                    }
                }

                bmp[row][col] = limitedColor[index];
            }
        }
